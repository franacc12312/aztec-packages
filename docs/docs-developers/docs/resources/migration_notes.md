...
+ let maybe_message_sender: Option<AztecAddress> = self.context.msg_sender();
```

The `self.context.msg_sender_unsafe` method has been dropped as its use can be replaced with the standard `self.context.msg_sender` function.
