...
describe('ValidatorClient', () => {
  ...
  describe('collectAttestations', () => {
    it('should timeout if we do not collect enough attestations in time', async () => {
      ...
      await expect(
        validatorClient.collectAttestations(proposal, 2, new Date(dateProvider.now() + 100), BlockNumber(1)),
    ...
    it('should collect attestations from its own validators', async () => {
      ...
      await expect(
        validatorClient.collectAttestations(proposal, 3, new Date(dateProvider.now() + 100), BlockNumber(1)),
      ).rejects.toThrow(AttestationTimeoutError);
    ...
    it('should filter out attestations with mismatched payload', async () => {
      ...
      await expect(
        validatorClient.collectAttestations(proposal, 2, new Date(dateProvider.now() + 1000), BlockNumber(1)),
      ).rejects.toThrow(AttestationTimeoutError);
  ...
  describe('attestToProposal', () => {
    ...
    describe('filestore blob upload', () => {
      ...
      const createValidatorWithFileStore = async () => {
        return await ValidatorClient.new(
          config,
          blockBuilder,
          epochCache,
          p2pClient,
