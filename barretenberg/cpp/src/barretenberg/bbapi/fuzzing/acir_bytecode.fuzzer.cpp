/**
 * @file acir_bytecode.fuzzer.cpp
 * @brief Fuzzer for ACIR bytecode deserialization (msgpack-compact format)
 *
 * Tests the parsing pipeline:
 *   Raw bytes -> deserialize_msgpack_compact -> Acir::Program -> AcirFormat
 *
 * Targets memory safety issues in msgpack parsing and ACIR structure handling.
 */
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    try {
        ...
        acir_format::circuit_buf_to_acir_format(std::move(buf));
